...
class Galaxybuild_ui:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.func is None:
            ...
            self.func = args[0]                                                  # Set the function
            self.__widget__ = GalaxyUIBuilder(self.func, **self.kwargs)                # Set the widget
            self.func.__dict__["__widget__"] = self.__widget__                   # Ensure function has access to widget
...
class GalaxyUIBuilder(BaseWidget, NBTool):
    ...
    def __init__(self, function_or_method, inputs, History_Data, GalInstace, ToolID, upload_callback=None,  **kwargs):
    ...
    def _param_defaults(sig):
        ...
        for param in sig.parameters.values():
            params.append({
                ...
                "default": GalaxyUIBuilder._safe_default(param.default),
                "value": GalaxyUIBuilder._safe_default(param.default),
                "description": param.annotation if param.annotation != inspect.Signature.empty else '',
                "hide": False,
                "type": GalaxyUIBuilder._guess_type(param.default),
                "kinds": None,
                "choices": GalaxyUIBuilder._choice_defaults(param),
                "id": None,
                "events": None
            })
