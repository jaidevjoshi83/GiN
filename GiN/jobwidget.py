...
try:
	from nbtools import EventManager
except:
	pass 

try:
	from nbtools import ToolManager, UIOutput
except:
	pass
...
class GalaxyJobWidget(GalaxyUIOutput):
    ...
    def __init__(self, job=None, gi=None, **kwargs):
        ...
        UIOutput.__init__(
            self, color=self.default_color, logo=self.default_logo, **kwargs
        )
        self.job = job
        self.gi = gi
    ...
    def poll(self):
        ...
        if self.job is not None:
            try:  # Attempt to load the job info from the Galaxy server
                self.gi.jobs.show_job(
                    self.job["jobs"][0]["id"], full_details=True
                )  #### changed
    ...
    def submitted_text(self):
        ...
        time = self.gi.jobs.show_job(self.job["jobs"][0]["id"], full_details=True)[
            "create_time"
        ]  # changed
    ...
    def files_list(self):
        ...
        if self.job is None:
            return
        outputs = self.gi.jobs.show_job(self.job["jobs"][0]["id"], full_details=True)[
            "outputs"
        ]
        rval = []
    ...
    def handle_notification(self):
        if self.status == "error":
            ToolManager.instance().send(
                "notification",
                {
                    "message": f"Job #{self.name} has an error!",
                    "sender": "Galaxy/GiN Notebook",
                },
            )
        elif self.status == "ok":
            ToolManager.instance().send(
                "notification",
                {
                    "message": f"Job #{self.name} is complete!",
                    "sender": "Galaxy/GiN Notebook",
                },
            )
    ...
    def status_text(self):  # ToDo
        ...
        if self.job is None:
            return ""  # Ensure the job has been set
        if (
            "hasError" in self.job.info["status"]
            and self.job.info["status"]["hasError"]
        ):
            return "Error"
        elif (
            "completedInGp" in self.job.info["status"]
            and self.job.info["status"]["completedInGp"]
        ):
            return "Completed"
        elif (
            "isPending" in self.job.info["status"]
            and self.job.info["status"]["isPending"]
        ):
            return "Pending"
        else:
            return "Running"
    ...
    def attach_sharing(self):
        if self.sharing_displayed:
            self.toggle_job_sharing()  # Display sharing if toggled on
        self.extra_menu_items = {
            **self.extra_menu_items,
            **{"Share Job": {"action": "method", "code": "toggle_job_sharing"}},
        }
    ...
    def build_sharing_controls(self):
        ...
        for g in perms["groups"]:
            d = Dropdown(
                description=g["id"], options=["Private", "Read", "Read & Write"]
            )
        ...
        def save_permissions(button):
            ...
            for g in group_widgets:
                if g.value == "Read & Write":
                    save_perms.append(
                        {"id": g.description, "read": True, "write": True}
                    )
                elif g.value == "Read":
                    save_perms.append(
                        {"id": g.description, "read": True, "write": False}
                    )
                else:
                    save_perms.append(
                        {"id": g.description, "read": False, "write": False}
                    )
    ...
    def toggle_job_sharing(self):
        ...
        if self.sharing_displayed:
            ...
            self.appendix.children = (
                self.sharing_displayed if self.sharing_displayed is not True else []
            )
            ...
            self.sharing_displayed = (
                self.appendix.children if self.appendix.children else True
            )
