...
export class GalaxyUIBuilderView extends BaseWidgetView {
    ...
    async add_tool_migration_button(index){
        ...
        const form =`<div class="migration-tool" >
                        <span id="migra-refresh-btn">Select Server<i class="fa fa-refresh" title="Refresh server list" id="migration-tool-button" style="margin-left:10px"></i></span>
                        <div class="form-restore-div" >
                            <div class="div-tool-migration">
                                <select class="tool-migration-select">
                                </select>
                            </div>

                            <div class="button-div" >
                                <button id="button-tool-refresh" >Restore Form for Current User</button>
                            </div>
                        </div>
                    </div>`

        const utm = new DOMParser().parseFromString( form, 'text/html').querySelector('.migration-tool')
        ...
        const Select = utm.querySelector('.tool-migration-select')
        const btn = utm.querySelector('#button-tool-refresh')
        ...
        btn.addEventListener('click', ()=>{
            ...
            this.section_new = cell.model.metadata.get('section')

            ContextManager.context().run_cell(cell)
        })

        this.el.querySelector('.nbtools-form').prepend(utm)
    }
    ...
    login_form(){
        ...
        var data_upload = `<div class="login-form"> 
                                ...
                                <div id="refresh-galaxy-cells-checkbox-dev" style="display: none; ">
                                    <span class="ui-form-title-text"> <b>Change tool origin to current Galaxy server</b> </span>
    ...
    add_galaxy_cell_metadata(galaxy_tool){
        ...
        for(var i = 0; i < cells.length; i++) {
            if (cells[i]._input.node.querySelector('.lm-Widget.p-Widget.jp-InputPrompt.jp-InputArea-prompt').innerText == '[*]:'){
                ...
                cells[i].model.metadata.set('nbtools', {'force_display':true})
            }
        }
    }

    async generate_tool_form (){
    ...
    new_form_data(form, params=[]){

        var self = this;
        for (var i = 0; i < form.children.length; i++) {
            if (  form.children[i].className.includes('ui-portlet-section section-row') || (form.children[i].className.includes('pl-2') && form.children[i].style.display == 'block')  || form.children[i].className.includes('ui-repeat section-row') ){
                params.concat(self.new_form_data(form.children[i], params))
            }
             else if (form.children[i].className == 'ui-form-element section-row sections' ) {
                params.concat(self.new_form_data(form.children[i], params))
            } 
    ...
    clean_param_for_job(data, checking){
        var cleaned_param = {}
                ...
                if(checking ){
                    if(!data[i]['optional']){
                        if (data[i]['data'] == null || Object.keys(data[i]['data']).length === 0){
                            ...
                            this.el.querySelector(`#${data[i]['id']}`).style.background = ''
                        }
                    }
                }

            }  
            cleaned_param[data[i].name] = data[i]['data']
        }
    ...
    drill_down(options, selected_values){
        ...
        for (var i = 0; i<options.length; i++){
            ...
            div4.className = 'Main'
            div4['data-key'] = {'optional':null, 'data':null, 'tool_type': 'drill_down'}
    ...
    return_drill_down_dict(drill_down){
        ...
        drill_down.querySelectorAll('input').forEach(function(input) {
            input.addEventListener('click', async (e)=>{
                ...
                var refine_inputs = await KernelSideDataObjects(`import IPython\nfrom GiN.taskwidget  import GalaxyTaskWidget\nclass Temp(object):\n    def Return(self):\n        return IPython.display.JSON(GalaxyTaskWidget.updated_form(${JSON.stringify(self.model.get('origin'))}, json.loads(base64.b64decode("${btoa(JSON.stringify(Inputs))}")), ${JSON.stringify(self.model.get('galaxy_tool_id'))}, ${JSON.stringify(history.value)}))\na = Temp()\na.Return()`)
    ...
    add_drill_down_section(input_def, FormParent, NamePrefix){
        ...
        row['data-key']  = {'optional': input_def.optional, 'name':NamePrefix+input_def['name'], 'data':input_def.value, 'id':input_def.id, 'tool_type':'drill_down'}
        ...
        selectspan.addEventListener('click', async () => {
            ...
            for (var i = 0; i < Div2.querySelectorAll('input').length; i++) {
                ...
                list.push(Div2.querySelectorAll('input')[i].value)
            }
    ...
    add_input_value (input_def, FormParent, NamePrefix){
        ...
        row['data-key'] =  {'name':NamePrefix+input_def['name'], 'data':input_def.value, 'optional':input_def.optional, 'id': input_def.id, 'tool_type': 'input_value' }
    ...
    async data_upload_tool() {
        ...
        var datatypes_genomes = this.model.get('inputs')
        ...
        for(var i = 0; i < datatypes_genomes[0].length; i++) {
            const opt = datatypes_genomes[0][i];
            const el = document.createElement("option");
            el.textContent = opt;
            el.value =  datatypes_genomes[0][i];;
        ...
        for(var i = 0; i < datatypes_genomes[1].length; i++) {
            const opt = datatypes_genomes[1][i][0];
            const el = document.createElement("option");
            el.textContent = opt;
            el.value =  datatypes_genomes[1][i][1];;
    ...
    async add_history_list(origin){
       ...
       var options
        ...
        if (options && options[0] && options[0]['id'] === undefined) {
            options = this.model.get('history_ids')
        }
        ...
        select.addEventListener("change", async (e) => {
            ...
            if (!ContextManager.notebook_tracker) return;               
            if (!ContextManager.notebook_tracker.currentWidget) return; 
            ContextManager.tool_registry.current.content.activeCell.model.metadata.set('selected_history_id', select.value)
    ...
    History_refresh(){
        ...
        async function historyRefresh(){
            ...
            if (self.el.querySelector('.tool-migration-select')){
                origin = self.el.querySelector('.tool-migration-select').value
            } else{
                origin = self.model.get('origin')
            }
    ...
    async AddHelpSection(help){
        ...
        HelpButton.addEventListener("click",  async function() {
            ...
            if (self.el.querySelector('#dataset-history-list')){
                history_id = self.el.querySelector('#dataset-history-list').value 
            } else {
                history_id = self.model.get('history_ids')[0]['id']
            }

            var help = await KernelSideDataObjects(`import IPython\nfrom GiN.taskwidget  import GalaxyTaskWidget\nclass Temp(object):\n    def Return(self):\n        return IPython.display.JSON(GalaxyTaskWidget.updated_form(server=${JSON.stringify(self.model.get('origin'))}, tool_id=${JSON.stringify(self.model.get('galaxy_tool_id'))}, history_id=${JSON.stringify(history_id)}))\na = Temp()\na.Return()`)
            KernelSideDataObjects(`try:\n    del a\nexcept:\n    print("a is not defined")`)

            HelpContent.innerHTML = help['help']
    ...
    add_repeat_section(input_def, FormParent, NamePrefix){
        ...
        InnerTitleDiv.className = 'inner-title-div'
        ...
        InnerTitleDiv.append(TitleSpan)

        title.append(InnerTitleDiv)
        ...
        function add_internal_repeat(inputs, count){
            ...
            InnerTitleDiv.append(InnerTitleSpan )
            InnerTitle.append(InnerTitleDiv)
            ...
            DeleteButton.addEventListener("click", async function(e){ 
                ...
                var refine_inputs = await KernelSideDataObjects(`import IPython\nfrom GiN.taskwidget  import GalaxyTaskWidget\nclass Temp(object):\n    def Return(self):\n        return IPython.display.JSON(GalaxyTaskWidget.updated_form(${JSON.stringify(origin)}, json.loads(base64.b64decode("${btoa(JSON.stringify(Inputs))}")), ${JSON.stringify(self.model.get('galaxy_tool_id'))}, ${JSON.stringify(history_id.value)}))\na = Temp()\na.Return()`)
                KernelSideDataObjects(`try:\n    del a\nexcept:\n    print("a is not defined")`)
    ...
    add_input_data(input_def, FormParent, NamePrefix, call_back_data={}){ 
        ...
        if(input_def.value ){
            ...
            row['data-key']['data']['values'] = input_def.default_value.values
        }
        else{
        ...
        Select.addEventListener("change", async (e) => {
            ...
            row['data-key']['data'] = {'batch': false, 'values': [JSON.parse(e.target.value)] }
            ...
            this.update_metadata_FormState('galaxy', refine_inputs['inputs'])

            if (Select.multiple === true ){
                if (input_def.multiple){
                    row['data-key']['data'] = {'batch': false, 'values': [] }
                } else{
                    row['data-key']['data'] = {'batch': true, 'values': [] }
                }
    ...
    add_select_field(input_def, FormParent, NamePrefix){
        ...
        if (input_def.display == 'checkboxes') {
            ...
            row['data-key']['tool_type'] = 'checkbox'
            ...
            selectspan.addEventListener('click', async () => {
                ...
                for (var i = 0; i < OuterDiv.querySelectorAll('input').length; i++ ) {
                    ...
                    list.push(OuterDiv.querySelectorAll('input')[i].value)
            ...
            row['data-key']['optional'] = true
            row['data-key']['tool_type'] = 'select_field'
    ...
    add_boolean_field (input_def, FormParent, NamePrefix ){
        ...
        row['data-key'] =  {'name':select.name, 'data':input_def.value, 'optional':input_def.optional, 'id':input_def.id, 'tool_type': 'boolean'}
        ...
        select.addEventListener('change', async (e) => {
            ...
            row['data-key'] =  {'name':input_def.name, 'data':e.target.value, 'optional':input_def.optional}
    ...
    async add_conditional_section(input_def, parent, NamePrefix, call_back_data={}){
        ...
        row.id = input_def.id
        row['data-key'] = {'data': input_def.value, 'optional': input_def.optional}
        row.append(title)
        row.append(select)
        parent.append(row)
        ...
        select.addEventListener("change", async (e) => {
            ...
            row['data-key'] = {'data': e.target.value, 'optional': input_def.optional}
    ...
    add_conditional_section_1(input_def, parent, NamePrefix, call_back_data={}){
        ...
        const select = document.createElement('select')
        select.name = NamePrefix+input_def['name']+"|"+input_def['test_param']['name']
        select.setAttribute('data-value', JSON.stringify({'optional': input_def['test_param'].optional, 'name': input_def.name, 'data':input_def['test_param'].value}))
        ...
        row['data-key'] =  {'data': input_def['test_param'].value, 'name': NamePrefix+input_def['name']+"|"+input_def['test_param']['name'], 'optional': input_def.test_param.optional, 'id': select.id, "tool_type": 'conditional'}
        row.style.display = 'block'
        row.id = input_def.id
        row.append(title)
        row.append(select)
   ...
   async add_section (input_def, parent, NamePrefix){
        ...
        Button.addEventListener("click", async (e) => {
            ...
            if(self.el.querySelector('#dataset-history-list')){
                ...
                history_id = this.model.get('history_id')
            }
            ...
            if(dataset['job_source_id']){
                for (let i = 0; i < Infinity; ++i) {
                    ...
                    if (state === undefined){
                        ...
                        else if(state['state'] == "request_failed"){
                            break
                        }
            ...
            for (let i = 0; i < Infinity; ++i) {
                ...
                if ( data['state'] == 'ok' ) {
                    ...
                    if (Tbl.querySelector('.fas.fa-spinner.fa-spin')){
                        Tbl.querySelector('.fas.fa-spinner.fa-spin').style.display = 'none'

                    }
                   if (Tbl.querySelector('.fa.fa-clock-o')){
                        Tbl.querySelector('.fa.fa-clock-o').style.display = 'none'
                   }
                    break;
                }  
    ...
    async JobStatusTemplate (parent, job){
        ...
        if (job['state'] != 'job failed') {
            template.querySelector('.job-status-footer').innerHTML = `<div style="padding:5px; font-size:12px;">Submitted by: <b> ${apiKey['email'] }</b> on <b>${job['create_time'].split('T')[0]}</b> at <b>${job['create_time'].split('T')[1].split('.')[0]}</div></b>`
